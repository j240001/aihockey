// ==========================================
// STRATEGY: BT8 - THE RELAY SYSTEM (With Sticky Debug)
// ==========================================
...
(function() {
    ...
    // --- HELPER: STICKY DEBUG TEXT ---
    // Forces text to stay on screen for 'frames' duration
    function setDebug(p, text, isImportant = false) {
        // 1. If we have a timer running, decrement it
        if (p.debugTimer > 0) {
            p.debugTimer--;
            // If the current message is important, don't overwrite it with boring stuff
        ...
        // 3. If this is an important action, set the timer (e.g., 60 frames = 1 second)
        if (isImportant) {
            p.debugTimer = 60; 
        }
    }
    ...
    function makeBB(p) {
            ...
            for (const m of teammates) {
                ...
                if (score > maxSafety) {
                    ...
                    if (typeof predictLeadPass === "function") {
                        const lead = predictLeadPass(p, m);
                        if (lead) passTargetLead = { x: lead.x, y: lead.y };
                    }
                }
            }
        }
        ...
        return {
            ...
            safestMate, shouldPass, 
            forwardTarget, passTargetLead,
    ...
    const actShoot = new ActionNode(bb => {
        // IMPORTANT: Sticky Text (true)
        setDebug(bb.p, "🔥 SHOOTING", true);
        return { tx: bb.enemyGoal, ty: RY, action: "shoot" };
    });

    const actDriveNet = new ActionNode(bb => {
        setDebug(bb.p, "Driving Net", false);
    ...
    const actSafePass = new ActionNode(bb => {
        ...
        const pt = bb.passTargetLead;
        if (!t) return null;
        
        setDebug(bb.p, "Safe Pass -> #" + t.id, true);
        puck.passTargetId = t.id; 

        if (pt) return { tx: pt.x, ty: pt.y, action: "pass", target: t };
