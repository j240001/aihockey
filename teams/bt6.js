...
(function() {
    ...
    const TREE_C = 
        new SelectorNode([
            ...
            const finalY = (bb.p.y < RY) ? carrier.y - 50 : carrier.y + 50;
    ...
    const TREE_LW = 
        new SelectorNode([
            ...
            const finalY = (bb.p.y < RY) ? carrier.y - -70 : carrier.y + -70;
            return { tx: finalX, ty: finalY, action:'none' };
        })
            ]),
            new SequenceNode([
    ...
    const TREE_LD = 
        new SelectorNode([
            ...
            new SequenceNode([
                ...
                condPuckInNeuZone,
                new ActionNode(bb => ({
            tx: bb.myGoalX + (bb.forwardDir * (undefined)),
            ty: RY,
            action:'none'
        }))
            ]),
            ...
            if (!carrier) return { tx: bb.p.x, ty: bb.p.y, action:'none' };
            const finalX = carrier.x + (bb.forwardDir * -20);
            const finalY = (bb.p.y < RY) ? carrier.y - -30 : carrier.y + -30;
            return { tx: finalX, ty: finalY, action:'none' };
        })
            ]),
            new SequenceNode([
